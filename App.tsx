...
const StoryboardEditor = forwardRef<StoryboardEditorHandle, { isApiReady: boolean }>(({ isApiReady }, ref) => {
        ...
        return (
             <div className="bg-gray-800 rounded-lg p-4 flex flex-col space-y-3">
                ...
                <textarea
                    ...
                    disabled={sceneCreatorMode === SceneCreatorMode.IDLE || !isApiReady}
                />
                <button onClick={() => handleGenerateScene(prompt)} disabled={!canCreate || isLoading || !isApiReady} className="btn-primary w-full">
        ...
        <>
            ...
            <div className="container mx-auto p-4 grid grid-cols-1 lg:grid-cols-12 gap-6 relative">
                 <aside className="lg:col-span-3 flex flex-col space-y-6">
                    <CreativeStudio onCreate={handleCreateAsset} disabled={isLoading || !isApiReady} />
                    <SceneCreator />
                </aside>
    ...
    return (
        <div className="flex flex-col space-y-4">
            <p className="text-sm text-gray-400">
                Vui lòng nhập khóa API Google Gemini của bạn. Khóa của bạn sẽ được lưu trữ an toàn trong trình duyệt của bạn và sẽ không bao giờ được chia sẻ.
            </p>
            <input
                type="password"
                value={inputKey}
            ...
            />
             <p className="text-xs text-gray-500">
                Bạn có thể lấy khóa API từ <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Google AI Studio</a>.
            </p>
            <div className="flex justify-end space-x-2">
                 {currentKey && <button onClick={onClose} className="btn-secondary">Hủy</button>}
                <button onClick={handleSave} className="btn-primary" disabled={!inputKey.trim()}>
                    Lưu Khóa API
                </button>
            </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-900 text-white font-sans">
        ...
        `}</style>
         <Modal
            ...
            onClose={() => { if (isApiReady) setIsApiModalOpen(false) }}
            title="Thiết Lập Khóa API Gemini"
        >
            <ApiKeySetup 
        ...
        <header className="bg-gray-800/50 backdrop-blur-sm p-4 fixed top-0 left-0 right-0 z-40 border-b border-gray-700">
            <div className="container mx-auto flex justify-between items-center">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    />
                    <button onClick={() => setIsApiModalOpen(true)} className="btn-secondary p-2" title="Cài đặt API Key">
                        <SettingsIcon />
                    </button>
                </div>
            </div>
        ...
        <main className="pt-24">
            <StoryboardEditor ref={storyboardActionsRef} isApiReady={isApiReady} />
        </main>
    </div>
  );
};
